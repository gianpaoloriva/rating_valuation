...
- [x] **Grafico unico distribuzione PD sul campione (pagina 6)**: nuova `pd_distribution()` in `backtest/comparator.py` — densità KDE su `log10(PD)` con floor 1e-5 (le PD = 0 si accumulano al floor) e conteggio delle mode via `scipy.signal.find_peaks` (prominence 5%); `PDDistribution.is_bimodal` rende misurabile la "doppia sella". La pagina Backtest Comparator mostra istogramma + KDE con il target evidenziato (linea tratteggiata) e warning se il campione è bimodale. Verificato sul campione reale FY2024 (256 società, 1 000 trial, ~0,4 s): bimodale confermata — moda al floor (144 società con PD ≈ 0) e seconda moda su PD > 10%, sella a ~0,1%. 6 test dedicati; replicato sul gemello (367 verdi).
