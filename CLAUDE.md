...
python3 data/etl/aida_to_companies.py                    # target: random draw, seed 42
python3 data/etl/aida_to_companies.py --target <id|piva> # flag a specific company as target
...
- **`src/rating_valuation/agentic_credit_risk/`** — four files:
  ...
  - `credit_metrics.py` — aggregates the Monte Carlo output into yearly default frequency, marginal PD, cumulative PD, LGD, EL, UL. The LGD waterfall enforces limited liability: negative simulated EV is floored at zero, LGD is capped at the unsecured EAD, and the recovery mean only counts defaults with material exposure — so `lgd_mean ≤ ead_mean` and `recovery ∈ [0, 1]` hold by construction even on distressed targets.
...
- **`companies.csv` contains both peers and target**: `is_target=1` marks the subject of the valuation; `peer_sample()` drops it by default (`exclude_target=True`). The primary (real) dataset has 276 peers + 1 target (`metal_d_s_r_l`, METAL D S.R.L. — chosen 2026-07-21 as the company closest to the sector medians with positive EBITDA/NWC and material debt; the original random draw `trafer_spa` was a broker and misleading) in `Metals Wholesale (ATECO 4672)` for 2020–2024; the synthetic fixture in `data/synthetic/` has 15 peers + 1 target (`riva_meccanica`) in `Industrial Machinery` for 2022–2024. Real-data caveat: the credit simulator requires a positive expected EBITDA margin — `from_company()` raises on operating losses; `BacktestRunner` skips and logs such companies (`BacktestResult.skipped`). Negative NWC is supported (the NWC Weibull location may go negative).
...
- `TODO.md` — single source of truth for the open backlog **and** completed functionality (replaces the old README "Stato di sviluppo" section). Ordered backlog-first: the "Da fare" section at the top is what remains (Appendix A integration into the simulator, optional 4b coverage-ratio default trigger, Section 5 backtest pending default labels, shared-package extraction — decided, installable package, twin-repo alignment rule until then); the "Completato" section at the bottom is the historical record (batches by date + the original 2026-04 audit items P1–P4, all closed).
